 ...
 * (Legacy function – you can still call this if you just want to push at the end.)
...
/**
 * Inserts a new value into the linked list at a specified position.
 * @param {string} value - The value to add.
 * @param {HTMLElement} container - The linked list container element.
 * @param {string} position - Insertion mode: "first", "last", "middle", or "index".
 * @param {number} [specificIndex] - Specific index (used when position is "index").
 */
function insertAt(value, container, position, specificIndex) {
  let insertionIndex;
  if (position === "first") {
    insertionIndex = 0;
  } else if (position === "last") {
    insertionIndex = linkedList.length;
  } else if (position === "middle") {
    insertionIndex = Math.floor(linkedList.length / 2);
  } else if (position === "index") {
    if (typeof specificIndex !== "number" || specificIndex < 0 || specificIndex > linkedList.length) {
      alert("Invalid index provided!");
      return;
    }
    insertionIndex = specificIndex;
  } else {
    alert("Invalid insertion mode!");
    return;
  }
  linkedList.splice(insertionIndex, 0, value);
  updateUI(container);
}
...
export { updateUI, push, insertAt, removeLast, removeByValue };
